...
- **私教预约系统**（教练私教档案、课时购买、预约/改期/取消、周课表拖拽交互）
...
erDiagram
    ...
    enrollment {
        ...
        int total_sessions "总课时数（私教）"
        int remaining_sessions "剩余课时数（私教）"
        datetime create_time "创建时间"
        datetime update_time "更新时间"
    }
...
- **URL**: `POST /api/private-coach/purchase`
- **功能**: 购买私教课时（扣减会员余额，增加教练余额）
...
- **URL**: `POST /api/private-coach/book-session`
- **功能**: 预约教练已发布的排课时段
...
- **URL**: `POST /api/private-coach/book-direct`
- **功能**: 直接预约指定时间段（创建新排课记录）
...
| `/member/private-courses` | member-private-courses | CoachListView.vue | 继承父路由守卫 |
