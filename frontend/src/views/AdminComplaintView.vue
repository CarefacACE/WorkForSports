  ...
  <div class="admin-complaint-page">
    <el-card shadow="never">
      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column prop="username" label="投诉会员" width="120" />
        <el-table-column prop="coachUsername" label="被投诉教练" width="120" />
    ...
    <el-dialog v-model="processDialogVisible" title="处理投诉" width="500px">
      <div v-if="currentItem" class="process-detail">
        <el-descriptions :column="2" border size="small">
          <el-descriptions-item label="投诉会员">{{ currentItem.username }}</el-descriptions-item>
          <el-descriptions-item label="被投诉教练">{{ currentItem.coachUsername }}</el-descriptions-item>
          ...
          <el-descriptions-item label="投诉内容" :span="2">{{ currentItem.content }}</el-descriptions-item>
        </el-descriptions>
      </div>
    ...
    <!-- 详情对话框 -->
    <el-dialog v-model="detailDialogVisible" title="投诉详情" width="500px">
...
import { listComplaints, processComplaint, type CoachComplaint } from '../api/feedback';
...
const total = ref(0);
const tableData = ref<CoachComplaint[]>([]);
const statusFilter = ref('');

const processDialogVisible = ref(false);
const detailDialogVisible = ref(false);
const currentItem = ref<CoachComplaint | null>(null);
...
async function fetchData() {
  loading.value = true;
  try {
    const result = await listComplaints(pageNum.value, pageSize.value, statusFilter.value || undefined);
...
function openProcessDialog(row: CoachComplaint) {
...
function viewDetail(row: CoachComplaint) {
  currentItem.value = row;
...
async function submitProcess() {
  ...
  try {
    await processComplaint(currentItem.value.id, processForm.status, processForm.feedback.trim());
